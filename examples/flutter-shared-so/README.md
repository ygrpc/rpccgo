...
- Flutter 通过 generated Dart FFI client 进入 Go `c-shared` runtime，再路由到 Flutter Dart message server。
...
Dart UI -> generated Dart FFI client -> librpccgo_flutter_shared.so -> generated Dart message server -> Flutter handler
```
...
`Dart Server Ping` 和 `Dart Server Start Stream` 按钮通过 generated Dart FFI client 进入 Go shared runtime，并路由到 Dart registered server。它验证的是 `Dart -> Go shared .so -> Dart message server -> Flutter handler`。

Dart message server callback 是 Dart isolate-local FFI callback；这个 example 不用 Kotlin/JNI 同步反调 Dart server。跨 Android/JNI 线程反调 Dart 需要另外设计异步边界，不和 Kotlin message server 使用同一套线程假设。
...
- `Dart Server Ping`：Flutter 通过 Dart FFI 调 `FlutterDevice.DescribeFlutter`，Go runtime 路由到 Dart message server。
- `Dart Server Start Stream`：Flutter 通过 Dart FFI 启动 `FlutterDevice.WatchFlutterEcho` server stream，每次 `Recv` 来自 Dart message server。
- `Dart Server Stop Stream`：Flutter 通过 Dart FFI cancel Dart-owned server stream。
