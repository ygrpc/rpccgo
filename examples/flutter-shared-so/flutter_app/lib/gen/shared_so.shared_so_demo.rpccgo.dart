// Code generated by protoc-gen-rpc-cgo-dart. DO NOT EDIT.
//
// Source: shared_so.proto

@ffi.DefaultAsset('package:rpccgofluttersharedso/gen/rpccgo.dart')
library;

import 'dart:convert' as convert;
import 'dart:ffi' as ffi;
import 'dart:typed_data' as typed_data;

import 'package:ffi/ffi.dart' as pkg_ffi;

import 'shared_so.pb.dart' as pb;

// rpccgo Dart FFI message client generated file for SharedSoDemo

typedef _RpccgoReleaseCAbi = ffi.Int32 Function(ffi.UintPtr ptr);
typedef _RpccgoTakeErrorTextCAbi =
    ffi.Int32 Function(
      ffi.Int32 errID,
      ffi.Pointer<ffi.UintPtr> textPtr,
      ffi.Pointer<ffi.Int32> textLen,
    );
typedef _RpccgoMessageUnaryCAbi =
    ffi.Int32 Function(
      ffi.UintPtr requestPtr,
      ffi.Int32 requestLen,
      ffi.Pointer<ffi.UintPtr> responsePtr,
      ffi.Pointer<ffi.Int32> responseLen,
    );
typedef _RpccgoStreamStartCAbi =
    ffi.Int32 Function(ffi.Pointer<ffi.Int32> handle);
typedef _RpccgoServerStreamStartCAbi =
    ffi.Int32 Function(
      ffi.UintPtr requestPtr,
      ffi.Int32 requestLen,
      ffi.Pointer<ffi.Int32> handle,
    );
typedef _RpccgoStreamSendCAbi =
    ffi.Int32 Function(
      ffi.Int32 handle,
      ffi.UintPtr requestPtr,
      ffi.Int32 requestLen,
    );
typedef _RpccgoStreamReadCAbi =
    ffi.Int32 Function(
      ffi.Int32 handle,
      ffi.Pointer<ffi.UintPtr> responsePtr,
      ffi.Pointer<ffi.Int32> responseLen,
    );
typedef _RpccgoStreamFinishCAbi =
    ffi.Int32 Function(
      ffi.Int32 handle,
      ffi.Pointer<ffi.UintPtr> responsePtr,
      ffi.Pointer<ffi.Int32> responseLen,
    );
typedef _RpccgoStreamFinishVoidCAbi = ffi.Int32 Function(ffi.Int32 handle);
typedef _RpccgoStreamCancelCAbi = ffi.Int32 Function(ffi.Int32 handle);

@ffi.Native<_RpccgoReleaseCAbi>(symbol: 'rpccgoRelease')
external int _rpccgoReleaseRaw(int ptr);

@ffi.Native<_RpccgoTakeErrorTextCAbi>(symbol: 'rpccgoTakeErrorText')
external int _rpccgoTakeErrorTextRaw(
  int errID,
  ffi.Pointer<ffi.UintPtr> textPtr,
  ffi.Pointer<ffi.Int32> textLen,
);

@ffi.Native<_RpccgoMessageUnaryCAbi>(
  symbol: 'rpccgoMsgFluttersharedv1SharedSoDemoComposeGreeting',
)
external int _composeGreetingRaw(
  int requestPtr,
  int requestLen,
  ffi.Pointer<ffi.UintPtr> responsePtr,
  ffi.Pointer<ffi.Int32> responseLen,
);

@ffi.Native<_RpccgoMessageUnaryCAbi>(
  symbol: 'rpccgoMsgFluttersharedv1SharedSoDemoIncrementRuntimeState',
)
external int _incrementRuntimeStateRaw(
  int requestPtr,
  int requestLen,
  ffi.Pointer<ffi.UintPtr> responsePtr,
  ffi.Pointer<ffi.Int32> responseLen,
);

@ffi.Native<_RpccgoMessageUnaryCAbi>(
  symbol: 'rpccgoMsgFluttersharedv1SharedSoDemoReadRuntimeState',
)
external int _readRuntimeStateRaw(
  int requestPtr,
  int requestLen,
  ffi.Pointer<ffi.UintPtr> responsePtr,
  ffi.Pointer<ffi.Int32> responseLen,
);

class SharedSoDemoRpccgoClient {
  const SharedSoDemoRpccgoClient();
  pb.ComposeGreetingResponse ComposeGreeting(
    pb.ComposeGreetingRequest request,
  ) {
    final responsePtr = pkg_ffi.calloc<ffi.UintPtr>();
    final responseLen = pkg_ffi.calloc<ffi.Int32>();
    final requestBytes = request.writeToBuffer();
    final requestPtr = _allocateBytes(requestBytes);
    try {
      final errID = _composeGreetingRaw(
        requestPtr.address,
        requestBytes.length,
        responsePtr,
        responseLen,
      );
      _throwIfError(errID);
      final responseBytes = _takeBytes(responsePtr.value, responseLen.value);
      return pb.ComposeGreetingResponse.fromBuffer(responseBytes);
    } finally {
      pkg_ffi.calloc.free(requestPtr);
      pkg_ffi.calloc.free(responsePtr);
      pkg_ffi.calloc.free(responseLen);
    }
  }

  pb.RuntimeStateResponse IncrementRuntimeState(
    pb.IncrementRuntimeStateRequest request,
  ) {
    final responsePtr = pkg_ffi.calloc<ffi.UintPtr>();
    final responseLen = pkg_ffi.calloc<ffi.Int32>();
    final requestBytes = request.writeToBuffer();
    final requestPtr = _allocateBytes(requestBytes);
    try {
      final errID = _incrementRuntimeStateRaw(
        requestPtr.address,
        requestBytes.length,
        responsePtr,
        responseLen,
      );
      _throwIfError(errID);
      final responseBytes = _takeBytes(responsePtr.value, responseLen.value);
      return pb.RuntimeStateResponse.fromBuffer(responseBytes);
    } finally {
      pkg_ffi.calloc.free(requestPtr);
      pkg_ffi.calloc.free(responsePtr);
      pkg_ffi.calloc.free(responseLen);
    }
  }

  pb.RuntimeStateResponse ReadRuntimeState(pb.ReadRuntimeStateRequest request) {
    final responsePtr = pkg_ffi.calloc<ffi.UintPtr>();
    final responseLen = pkg_ffi.calloc<ffi.Int32>();
    final requestBytes = request.writeToBuffer();
    final requestPtr = _allocateBytes(requestBytes);
    try {
      final errID = _readRuntimeStateRaw(
        requestPtr.address,
        requestBytes.length,
        responsePtr,
        responseLen,
      );
      _throwIfError(errID);
      final responseBytes = _takeBytes(responsePtr.value, responseLen.value);
      return pb.RuntimeStateResponse.fromBuffer(responseBytes);
    } finally {
      pkg_ffi.calloc.free(requestPtr);
      pkg_ffi.calloc.free(responsePtr);
      pkg_ffi.calloc.free(responseLen);
    }
  }

  ffi.Pointer<ffi.Uint8> _allocateBytes(List<int> data) {
    final ptr = pkg_ffi.calloc<ffi.Uint8>(data.length);
    ptr.asTypedList(data.length).setAll(0, data);
    return ptr;
  }

  typed_data.Uint8List _takeBytes(int ptr, int length) {
    if (length < 0) {
      throw StateError('rpccgo: response length is negative');
    }
    if (length == 0) {
      return typed_data.Uint8List(0);
    }
    if (ptr == 0) {
      throw StateError('rpccgo: response pointer is nil');
    }
    final data = typed_data.Uint8List.fromList(
      ffi.Pointer<ffi.Uint8>.fromAddress(ptr).asTypedList(length),
    );
    final status = _rpccgoReleaseRaw(ptr);
    if (status != 0) {
      throw StateError('rpccgo: failed to release response buffer');
    }
    return data;
  }

  void _throwIfError(int errID) {
    if (errID == 0) {
      return;
    }
    throw StateError(_takeError(errID));
  }

  String _takeError(int errID) {
    final textPtr = pkg_ffi.calloc<ffi.UintPtr>();
    final textLen = pkg_ffi.calloc<ffi.Int32>();
    try {
      final status = _rpccgoTakeErrorTextRaw(errID, textPtr, textLen);
      if (status != 0) {
        return 'rpccgo error $errID';
      }
      final data = _takeBytes(textPtr.value, textLen.value);
      return convert.utf8.decode(data);
    } finally {
      pkg_ffi.calloc.free(textPtr);
      pkg_ffi.calloc.free(textLen);
    }
  }
}
