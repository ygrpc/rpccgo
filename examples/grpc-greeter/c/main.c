...
static void run_native_unary_demo(void) {
  ...
  const char *name = "grpc-c";

  assert_status_ok(rpccgo_native_greeterv1_Greeter_SayHello((uintptr_t)name, 6, 0, &message_ptr, &message_len, &message_ownership),
                   "native unary error:");
...
static void run_output_error_demo(void) {
  const char *name = "grpc-c";
  int32_t err_id = rpccgo_native_greeterv1_Greeter_SayHello((uintptr_t)name, 6, 0, NULL, NULL, NULL);
  if (err_id == 0) {
