...
func (a *greeterCGOMessageAdapter) Broadcast(ctx context.Context, req *v1.SayHelloRequest, stream rpcruntime.ServerStreamingServer[*v1.SayHelloResponse]) error {
	...
	for {
		resp, err, stopped := greeterAwaitCGOMessageRecv(ctx, stream.FinishRequested(), func() (*v1.SayHelloResponse, error) { return session.Recv(ctx) }, finishSession, func() error { return session.Cancel(ctx) })
		if stopped {
			return err
		}
		if err != nil {
