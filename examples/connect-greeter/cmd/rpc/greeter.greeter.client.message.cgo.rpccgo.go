...
func FinishGreeterBroadcastMessageServerStream(ctx context.Context, handle int32) int32 {
	...
	var err error
	err = proto.NewGreeterBroadcastMessageStream(rpcruntime.StreamHandle(handle)).Finish(ctx)
	if err != nil {
		return int32(rpcruntime.StoreError(err))
	}
	return 0
...
func FinishGreeterChatMessageBidiStream(ctx context.Context, handle int32) int32 {
	...
	var err error
	err = proto.NewGreeterChatMessageStream(rpcruntime.StreamHandle(handle)).Finish(ctx)
	if err != nil {
		return int32(rpcruntime.StoreError(err))
	}
	return 0
