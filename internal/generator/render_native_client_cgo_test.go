...
func TestRenderNativeClientCGOSupportsRepeatedNativeABI(t *testing.T) {
	...
	for _, fragment := range []string{
		...
		"Scores, err = rpcruntime.NewRpcRepeatChecked((*int32)(unsafe.Pointer(input.ScoresPtr)), input.ScoresLen, input.ScoresOwnership > 0)",
		"if input.CountsPtr == 0 || input.CountsLen == 0 {",
		"Counts = rpcruntime.EmptyRpcRepeat[int64]()",
		"Counts, err = rpcruntime.NewRpcRepeatChecked((*int64)(unsafe.Pointer(input.CountsPtr)), input.CountsLen, input.CountsOwnership > 0)",
		"if input.RatiosPtr == 0 || input.RatiosLen == 0 {",
		"Ratios = rpcruntime.EmptyRpcRepeat[float64]()",
		"Ratios, err = rpcruntime.NewRpcRepeatChecked((*float64)(unsafe.Pointer(input.RatiosPtr)), input.RatiosLen, input.RatiosOwnership > 0)",
		"if input.MoodsPtr == 0 || input.MoodsLen == 0 {",
		"Moods = rpcruntime.EmptyRpcRepeat[int32]()",
		"Moods, err = rpcruntime.NewRpcRepeatChecked((*int32)(unsafe.Pointer(input.MoodsPtr)), input.MoodsLen, input.MoodsOwnership > 0)",
		"if input.FlagsPtr == 0 || input.FlagsLen == 0 {",
		"Flags = rpcruntime.EmptyRpcBoolRepeat()",
		"Flags, err = rpcruntime.NewRpcBoolRepeatChecked((*byte)(unsafe.Pointer(input.FlagsPtr)), input.FlagsLen, input.FlagsOwnership > 0)",
