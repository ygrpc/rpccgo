...
func TestRenderNativeClientCGOStreamsUseRuntimeStreamOperations(t *testing.T) {
	...
	for _, fragment := range []string{
		...
		"func rpccgoNativeTestv1GreeterListClose(stream C.int32_t) C.int32_t {",
		"func rpccgoNativeTestv1GreeterChatClose(stream C.int32_t) C.int32_t {",
	...
	assertGeneratedFileContentDoesNotContain(t, plugin, nativeClientFile,
		...
		"rpccgoNativeTestv1GreeterUploadClose",
