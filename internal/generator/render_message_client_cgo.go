...
func renderMessageClientStreamingClient(g *protogen.GeneratedFile, plan FilePlan, service ServicePlan, method MethodPlan, servicePackage string) {
	...
	renderMessageClientStreamExecutorPrefix(g, service, method, servicePackage, "Send")
	g.P("return session.Send(ctx, req)")
	renderMessageClientStreamExecutorSuffix(g)
	g.P("if err != nil {")
	g.P("return int32(rpcruntime.StoreError(err))")
	g.P("}")
	g.P("return 0")
	...
	g.P("var resp []byte")
	g.P("var err error")
	renderMessageClientStreamExecutorPrefix(g, service, method, servicePackage, "Finish")
	g.P("var finishErr error")
	g.P("resp, finishErr = session.Finish(ctx)")
	...
	renderMessageClientContextPrefix(g)
	g.P("var err error")
	renderMessageClientStreamExecutorPrefix(g, service, method, servicePackage, "Cancel")
...
func renderMessageServerStreamingClient(g *protogen.GeneratedFile, plan FilePlan, service ServicePlan, method MethodPlan, servicePackage string) {
	...
	g.P("var resp []byte")
	g.P("var err error")
	renderMessageClientStreamExecutorPrefix(g, service, method, servicePackage, "Receive")
	g.P("var recvErr error")
	g.P("resp, recvErr = session.Recv(ctx)")
	g.P("return recvErr")
	renderMessageClientStreamExecutorSuffix(g)
	g.P("if err != nil {")
	g.P("return int32(rpcruntime.StoreError(err))")
	...
	renderMessageClientContextPrefix(g)
	g.P("var err error")
	renderMessageClientStreamExecutorPrefix(g, service, method, servicePackage, "Done")
...
func renderMessageBidiStreamingClient(g *protogen.GeneratedFile, plan FilePlan, service ServicePlan, method MethodPlan, servicePackage string) {
	...
	renderMessageClientContextPrefix(g)
	g.P("var err error")
	renderMessageClientStreamExecutorPrefix(g, service, method, servicePackage, "CloseSend")
...
func renderMessageClientStreamExecutorPrefix(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan, servicePackage, operation string) {
	g.P("err = rpcruntime.DispatcherStream", operation, "[", servicePackage, service.GoName, "ActiveAdapter, ", messageClientMessageStreamSessionName(service, method, servicePackage), "](", servicePackage, service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", messageClientMessageStreamSessionName(service, method, servicePackage), ") error {")
}

func renderMessageClientStreamExecutorSuffix(g *protogen.GeneratedFile) {
	g.P("})")
}
...
func renderMessageServerStreamingCExportWrappers(g *protogen.GeneratedFile, plan FilePlan, service ServicePlan, method MethodPlan) {
	...
	doneName := messageCExportFuncName(plan, service, method, "done")
	g.P("//export ", doneName)
