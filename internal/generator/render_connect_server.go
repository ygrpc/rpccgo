...
func renderConnectServerStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("terminal := rpcruntime.NewDispatcherStreamTerminal[", service.GoName, "ActiveAdapter, ", service.GoName, method.GoName, "MessageStreamSession](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), errors.New(\"rpccgo: connect message stream handle is invalid\"))")
	...
	g.P("},")
	g.P("func() error {")
	g.P("return terminal.End(func(session ", service.GoName, method.GoName, "MessageStreamSession) error { return session.Done(ctx) })")
	g.P("},")
	g.P("func() error {")
	g.P("return terminal.End(func(session ", service.GoName, method.GoName, "MessageStreamSession) error { return session.Cancel(ctx) })")
	g.P("},")
	g.P(")")
	g.P("}")
	g.P()
