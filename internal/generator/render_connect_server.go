...
func renderConnectClientStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("if err != nil {")
	renderDispatcherStreamCancelIfPresent(g, service.GoName, sessionType, "handle", "ctx")
	...
	g.P("return session.Send(ctx, reqData)")
	g.P("}); err != nil {")
	renderDispatcherStreamCancelIfPresent(g, service.GoName, sessionType, "handle", "ctx")
	g.P("return nil, err")
	g.P("}")
	...
	g.P("respData, finishErr = session.Finish(ctx)")
...
func renderConnectServerStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("respData, recvErr = session.Recv(ctx)")
	g.P("return recvErr")
	g.P("})")
	...
	g.P("},")
	g.P("func() error {")
	g.P("return rpcruntime.DispatcherStreamDone[", service.GoName, "ActiveAdapter, ", sessionType, "](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", sessionType, ") error { return session.Done(ctx) })")
	g.P("},")
	g.P("func() error {")
	g.P("return rpcruntime.DispatcherStreamCancel[", service.GoName, "ActiveAdapter, ", sessionType, "](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", sessionType, ") error { return session.Cancel(ctx) })")
	g.P("},")
	g.P(")")
	g.P("}")
	g.P()
...
func renderConnectBidiStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("}")
	g.P("return rpcruntime.DispatcherStreamSend[", service.GoName, "ActiveAdapter, ", sessionType, "](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", sessionType, ") error { return session.Send(ctx, reqData) })")
	g.P("},")
	g.P("func() error {")
	g.P("return rpcruntime.DispatcherStreamCloseSend[", service.GoName, "ActiveAdapter, ", sessionType, "](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", sessionType, ") error { return session.CloseSend(ctx) })")
	g.P("},")
	g.P("func() ([]byte, error) {")
	g.P("var respData []byte")
	g.P("err := rpcruntime.DispatcherStreamReceive[", service.GoName, "ActiveAdapter, ", sessionType, "](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", sessionType, ") error {")
