...
func renderConnectClientStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("if !ok {")
	g.P(`return nil, errors.New("rpccgo: connect message stream handle is invalid")`)
	g.P("}")
...
func renderConnectServerStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("session, ok := rpcruntime.LoadDispatcherStream[", service.GoName, "ActiveAdapter, ", service.GoName, method.GoName, "MessageStreamSession](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle))")
	g.P("if !ok {")
	g.P(`return errors.New("rpccgo: connect message stream handle is invalid")`)
	g.P("}")
	g.P("for {")
...
func renderConnectBidiStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("terminal, ok := rpcruntime.TakeDispatcherStream[", service.GoName, "ActiveAdapter, ", service.GoName, method.GoName, "MessageStreamSession](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle))")
	g.P("if !ok {")
	g.P(`finishErr = errors.New("rpccgo: connect message stream handle is invalid")`)
	g.P("return")
	g.P("}")
	g.P("if done {")
	g.P("finishErr = terminal.Done(ctx)")
	g.P("return")
	g.P("}")
	g.P("finishErr = terminal.Cancel(ctx)")
	g.P("})")
