...
func renderConnectClientStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("return nil, err")
	g.P("}")
	g.P("session, ok := bridge.Load", method.GoName, "MessageStream(handle)")
	g.P("if !ok {")
	...
	g.P("if err != nil {")
	g.P("if terminal, ok := bridge.Take", method.GoName, "MessageStream(handle); ok {")
	...
	g.P("return nil, err")
	g.P("}")
	g.P("terminal, ok := bridge.Take", method.GoName, "MessageStream(handle)")
	g.P("if !ok {")
	g.P(`return nil, errors.New("rpccgo: connect message stream handle is invalid")`)
	g.P("}")
