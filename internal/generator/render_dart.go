...
func renderDartCallbackCallableSetup(g *protogen.GeneratedFile, method MethodPlan) {
	...
	dartP(g, 3, "}")
	dartP(g, 2, "}")
	dartP(g, 2, "onRecvNative = ffi.NativeCallable<_RpccgoMessageOnRecvCAbi>.listener((stream, responsePtr, responseLen) {")
	...
	dartP(g, 3, "}")
	dartP(g, 2, "});")
	dartP(g, 2, "onDoneNative = ffi.NativeCallable<_RpccgoMessageOnDoneCAbi>.listener((stream, errID) {")
