...
func TestRenderMessageClientCGODefinesUnaryExportSurface(t *testing.T) {
	...
	for _, fragment := range []string{
		...
		`rpcruntime.RequireDispatcherStream[v1.GreeterActiveAdapter, v1.GreeterUploadMessageStreamSession](v1.GreeterDispatcherForRuntime(), rpcruntime.StreamHandle(handle), errors.New("rpccgo: message client stream handle is invalid"))`,
		`rpcruntime.TakeRequiredDispatcherStream[v1.GreeterActiveAdapter, v1.GreeterUploadMessageStreamSession](v1.GreeterDispatcherForRuntime(), rpcruntime.StreamHandle(handle), errors.New("rpccgo: message client stream handle is invalid"))`,
