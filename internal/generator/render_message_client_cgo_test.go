...
func TestRenderMessageClientCGODefinesUnaryExportSurface(t *testing.T) {
	...
	for _, fragment := range []string{
		...
		"static inline void callGreeterRpccgoMessageOnRecvCallback",
		"static inline void callGreeterRpccgoMessageOnDoneCallback",
		"func rpccgoMsgTestv1GreeterListStart(requestPtr C.uintptr_t, requestLen C.int32_t, handle *C.int32_t, onRecv C.GreeterRpccgoMessageOnRecvCallback, onDone C.GreeterRpccgoMessageOnDoneCallback) C.int32_t {",
		...
		"func rpccgoMsgTestv1GreeterListCancel(handle C.int32_t) C.int32_t {",
		"callbackState, _ := rpcruntime.StreamCallbackReceiveState(rpcruntime.StreamHandle(handleValue))",
		"callbackState.MarkCanceled()",
		"err := v1.GreeterMessageListCancel(ctx, rpcruntime.StreamHandle(handleValue))",
		"callbackState.WaitDone()",
		"// rpccgoMsgTestv1GreeterListClose closes callback receive ownership for the message server-streaming client entrypoint for test.v1.Greeter.List without delivering further callbacks.",
		"func rpccgoMsgTestv1GreeterListClose(handle C.int32_t) C.int32_t {",
		"callbackState.MarkCallbackReceiveClosed()",
		"C.callGreeterRpccgoMessageOnRecvCallback",
		"C.callGreeterRpccgoMessageOnDoneCallback",
		"//export rpccgoMsgTestv1GreeterChatCloseSend",
		"func rpccgoMsgTestv1GreeterChatCloseSend(handle C.int32_t) C.int32_t {",
		"func rpccgoMsgTestv1GreeterChatClose(handle C.int32_t) C.int32_t {",
	...
	assertGeneratedFileContentDoesNotContain(t, plugin, cgoClientFile,
		...
		"rpccgoMsgTestv1GreeterUploadClose",
		...
		"typedef int32_t (*GreeterRpccgoMessageOnRecvCallback)",
		"typedef int32_t (*GreeterRpccgoMessageOnDoneCallback)",
		"errID := int32(C.callGreeterRpccgoMessageOnRecvCallback",
		"_ = C.callGreeterRpccgoMessageOnDoneCallback",
	)
}
