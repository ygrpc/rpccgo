...
func renderRuntimeRegistrationForSource(g *protogen.GeneratedFile, ctx runtimeRegistrationRenderContext, source ActiveRecordSourcePlan) {
	...
	switch {
	case source.Contract == ActiveRecordContractNative && source.Origin == ActiveRecordOriginGo:
		...
		g.P("}")
		g.P()
	case source.Contract == ActiveRecordContractNative && source.Origin == ActiveRecordOriginCGO:
		g.P("func ", source.RegisterName, "(", source.InputName, " ", ctx.nativeAdapterName, ") error {")
		g.P("return register", serviceName, "GoNativeServer(", source.SourceExpr, ")")
		g.P("}")
		g.P()
	case source.Contract == ActiveRecordContractMessage:
		...
		g.P("}")
		g.P()
	case source.Contract == ActiveRecordContractTransportMessage:
