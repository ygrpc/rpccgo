...
func renderCGOMessageServerServerStreamAdapter(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan, adapterName string) {
	...
	g.P("}")
	g.P("for {")
	g.P("resp, err, stopped := ", lowerInitial(service.GoName), "AwaitCGOMessageRecv(ctx, stream.FinishRequested(), func() (", messageGoPointerType(g, method.Response), ", error) { return session.Recv(ctx) }, func() error { return session.Finish(ctx) }, func() error { return session.Cancel(ctx) })")
	g.P("if stopped {")
	g.P("return err")
	g.P("}")
	g.P("if err != nil {")
