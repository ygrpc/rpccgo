...
func TestRenderConnectServerFileEmitsHandlers(t *testing.T) {
	...
	for _, fragment := range []string{
		...
		"handle, err := StartAllServiceMessageClientStream(ctx)",
		...
		"handle, err := StartAllServiceMessageServerStream(ctx, reqData)",
		...
		"handle, err := StartAllServiceMessageBidiStream(ctx)",
