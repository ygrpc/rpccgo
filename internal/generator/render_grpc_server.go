...
func renderGRPCClientStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	sessionType := service.GoName + method.GoName + "MessageStreamSession"
	...
	g.P("break")
	g.P("}")
	renderDispatcherStreamCancelIfPresent(g, service.GoName, sessionType, "handle", "stream.Context()")
	g.P("return err")
	g.P("}")
	g.P("reqData, err := proto.Marshal(req)")
	g.P("if err != nil {")
	...
	g.P("}")
	g.P("if err := rpcruntime.DispatcherStreamSend[", service.GoName, "ActiveAdapter, ", sessionType, "](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", sessionType, ") error {")
	g.P("return session.Send(stream.Context(), reqData)")
	g.P("}); err != nil {")
	renderDispatcherStreamCancelIfPresent(g, service.GoName, sessionType, "handle", "stream.Context()")
	g.P("return err")
	g.P("}")
	g.P("}")
	g.P("var respData []byte")
	g.P("err = rpcruntime.DispatcherStreamFinish[", service.GoName, "ActiveAdapter, ", sessionType, "](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", sessionType, ") error {")
	g.P("var finishErr error")
	g.P("respData, finishErr = session.Finish(stream.Context())")
...
func renderGRPCServerStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("func() ([]byte, error) {")
	g.P("var respData []byte")
	g.P("err := rpcruntime.DispatcherStreamReceive[", service.GoName, "ActiveAdapter, ", sessionType, "](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", sessionType, ") error {")
	g.P("var recvErr error")
	g.P("respData, recvErr = session.Recv(stream.Context())")
	g.P("return recvErr")
	g.P("})")
	g.P("return respData, err")
	g.P("},")
	...
	g.P("},")
	g.P("func() error {")
	g.P("return rpcruntime.DispatcherStreamDone[", service.GoName, "ActiveAdapter, ", sessionType, "](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", sessionType, ") error { return session.Done(stream.Context()) })")
	g.P("},")
	g.P("func() error {")
	g.P("return rpcruntime.DispatcherStreamCancel[", service.GoName, "ActiveAdapter, ", sessionType, "](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", sessionType, ") error { return session.Cancel(stream.Context()) })")
	g.P("},")
	g.P(")")
	g.P("}")
	g.P()
...
func renderGRPCBidiStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("}")
	g.P("return rpcruntime.DispatcherStreamSend[", service.GoName, "ActiveAdapter, ", sessionType, "](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", sessionType, ") error { return session.Send(stream.Context(), reqData) })")
	g.P("},")
	g.P("func() error {")
	g.P("return rpcruntime.DispatcherStreamCloseSend[", service.GoName, "ActiveAdapter, ", sessionType, "](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", sessionType, ") error { return session.CloseSend(stream.Context()) })")
	g.P("},")
	g.P("func() ([]byte, error) {")
	g.P("var respData []byte")
	g.P("err := rpcruntime.DispatcherStreamReceive[", service.GoName, "ActiveAdapter, ", sessionType, "](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), func(session ", sessionType, ") error {")
