...
func renderGRPCClientStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("return err")
	g.P("}")
	renderDispatcherStreamLoadSession(g, service.GoName, service.GoName+method.GoName+"MessageStreamSession", `errors.New("rpccgo: grpc message stream handle is invalid")`, "handle")
	g.P("for {")
	...
	g.P("break")
	g.P("}")
	renderDispatcherStreamCancelIfPresent(g, service.GoName, service.GoName+method.GoName+"MessageStreamSession", "handle", "stream.Context()")
	g.P("return err")
	g.P("}")
	g.P("reqData, err := proto.Marshal(req)")
	g.P("if err != nil {")
	...
	g.P("}")
	g.P("}")
	renderDispatcherStreamTakeSession(g, service.GoName, service.GoName+method.GoName+"MessageStreamSession", `errors.New("rpccgo: grpc message stream handle is invalid")`, "handle", "terminal")
...
func renderGRPCServerStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("}")
	renderDispatcherStreamLoadSession(g, service.GoName, service.GoName+method.GoName+"MessageStreamSession", `errors.New("rpccgo: grpc message stream handle is invalid")`, "handle")
	g.P("terminal := rpcruntime.NewDispatcherStreamTerminal[", service.GoName, "ActiveAdapter, ", service.GoName, method.GoName, "MessageStreamSession](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle), errors.New(\"rpccgo: grpc message stream handle is invalid\"))")
	g.P("return rpcruntime.RunServerStream(")
	g.P("func() ([]byte, error) {")
	g.P("return session.Recv(stream.Context())")
	g.P("},")
	...
	g.P("},")
	g.P("func() error {")
	g.P("return terminal.End(func(session ", service.GoName, method.GoName, "MessageStreamSession) error { return session.Done(stream.Context()) })")
	g.P("},")
	g.P("func() error {")
	g.P("return terminal.End(func(session ", service.GoName, method.GoName, "MessageStreamSession) error { return session.Cancel(stream.Context()) })")
	g.P("},")
	g.P(")")
	g.P("}")
	g.P()
...
func renderGRPCBidiStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("},")
	g.P("func() error {")
	g.P("return session.CloseSend(stream.Context())")
	g.P("},")
	g.P("func() ([]byte, error) {")
	g.P("return session.Recv(stream.Context())")
	g.P("},")
