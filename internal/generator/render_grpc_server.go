...
func renderGRPCBidiStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("}")
	renderDispatcherStreamLoadSession(g, service.GoName, service.GoName+method.GoName+"MessageStreamSession", `errors.New("rpccgo: grpc message stream handle is invalid")`, "handle")
	renderDispatcherStreamFinishOnce(g, service.GoName, service.GoName+method.GoName+"MessageStreamSession", `errors.New("rpccgo: grpc message stream handle is invalid")`, "handle", "stream.Context()")
	...
	g.P("if err != nil {")
	g.P("if errors.Is(err, io.EOF) {")
	g.P("receiveErrCh <- session.CloseSend(stream.Context())")
	g.P("return")
	g.P("}")
	...
	g.P("reqData, err := proto.Marshal(req)")
	g.P("if err != nil {")
	g.P(`receiveErrCh <- status.Errorf(codes.Internal, "rpccgo: grpc bidi request protobuf marshal failed: %v", err)`)
	g.P("return")
	g.P("}")
	g.P("if err := session.Send(stream.Context(), reqData); err != nil {")
	g.P("receiveErrCh <- err")
	...
	g.P("resp := new(", respType, ")")
	g.P("if err := proto.Unmarshal(respData, resp); err != nil {")
	g.P(`sendErrCh <- status.Errorf(codes.Internal, "rpccgo: grpc bidi response protobuf unmarshal failed: %v", err)`)
	g.P("return")
	g.P("}")
	g.P("if err := stream.Send(resp); err != nil {")
	g.P("sendErrCh <- err")
