...
func renderGRPCBidiStreamingImplementation(g *protogen.GeneratedFile, service ServicePlan, method MethodPlan) {
	...
	g.P("terminal, ok := rpcruntime.TakeDispatcherStream[", service.GoName, "ActiveAdapter, ", service.GoName, method.GoName, "MessageStreamSession](", service.GoName, "DispatcherForRuntime(), rpcruntime.StreamHandle(handle))")
	g.P("if !ok {")
	g.P(`finishErr = errors.New("rpccgo: grpc message stream handle is invalid")`)
	g.P("return")
	g.P("}")
	g.P("if done {")
	g.P("finishErr = terminal.Done(stream.Context())")
	g.P("return")
	g.P("}")
	g.P("finishErr = terminal.Cancel(stream.Context())")
	g.P("})")
