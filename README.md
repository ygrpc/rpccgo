...
`protoc-gen-rpc-cgo-dart` 生成 `<proto>.<service>.rpccgo.dart`，导入同目录的 `*.pb.dart` 类型，直接通过 `dart:ffi` 调用 rpccgo 生成的 message cgo client exports，例如 `rpccgo_msg_<package>_<Service>_<Method>`。公开 Dart 方法名保持 proto method 原名，不做 lowerCamelCase 改写。运行时需要加载由 rpccgo cgo package 构建出的 shared library，并在 Dart 依赖中包含 `ffi` package。
