...
func StoreErrorMsg(msg []byte) int {
	startCleanerOnce.Do(startCleaner)

	id := int(nextErrorID.Add(1))
...
func GetErrorMsgBytes(errorID int) (msg []byte, ok bool) {
	if errorID <= 0 {
		return nil, false
	} else {
		now := time.Now()
