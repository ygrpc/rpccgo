...
func generateMainFile(gen *protogen.Plugin) *protogen.GeneratedFile {
	// 1. Generate cgo_helpers.go (contains C exports, requires CGO)
	g := gen.NewGeneratedFile("cgo_helpers.go", "")
	...
	g.P("func Ygrpc_SetProtocol(protocol int) uint64 {")
	g.P("    switch protocol {")
	g.P("    case 0:")
	...
	g.P("            return uint64(rpcruntime.StoreError(err))")
	g.P("        }")
	g.P("        return 0")
	g.P("    case 2:")
	...
	g.P("        return 0")
	g.P("    default:")
	g.P("        return uint64(rpcruntime.StoreError(rpcruntime.ErrUnknownProtocol))")
	g.P("    }")
	g.P("}")
	g.P()
	...
	g.P("    *msgPtr = buf")
	g.P("    *msgLen = len(msg)")
	g.P("    *msgFree = (unsafe.Pointer)(C.Ygrpc_Free)")
	g.P("    return 0")
	g.P("}")
	g.P()

	// 2. Generate main.go (Pure Go entry point)
	gm := gen.NewGeneratedFile("main.go", "")
	gm.P("// Code generated by protoc-gen-rpc-cgo. DO NOT EDIT.")
	gm.P()
	gm.P("package main")
	gm.P()
	gm.P("func main() {}")
