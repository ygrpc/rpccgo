...
rpccgo removes `rpcruntime.Dispatcher[T]`, runtime bridge structs, stream executors, `StreamEntry`, and registry lifecycle helpers. Generated service runtime owns service-local registration, active binding closure construction, native/message conversion, final sessions, and stream registry binding. Runtime core only provides reusable primitives.

Each service has generated typed atomic pointers storing immutable service-local native and message active bindings, plus one non-generic stream registry storing final session values directly. Each final session embeds the small `StreamLifecycle` state primitive and the operation closures required by its capability set. Generated package-level functions load the binding or session and perform the operation directly.
...
- Generated service runtime uses typed `atomic.Pointer[binding]` slots directly; `rpcruntime.ActiveServerSlot`, `ServerKind`, `ServerContract`, `AdapterSnapshot`, and version metadata should be deleted.
