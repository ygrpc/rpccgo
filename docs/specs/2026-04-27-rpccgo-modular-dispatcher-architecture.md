...
active server 是 runtime bridge 的下游服务实现。不同 server kind 可以是 Go native server、C callback、connect-go handler、grpc-go server，或者 remote adapter。bridge 按 kind 直接调用对应 concrete server；只有 C callback 与 remote client 这类非标准 server shape 需要 generated adapter。
...
每个 server registration API 都只做一件事：把用户提供的 server、handler、callback adapter 或 remote adapter 写入 active server slot。connect-go handler 和 grpc-go server 直接注册，不再先包装成 service-specific message adapter。
...
标准 connect/grpc 监听入口由用户使用 `protoc-gen-connect-go` 或 `protoc-gen-go-grpc` 生成物自行搭建。rpccgo 不生成本地 connect/grpc transport handler；C 端调用进入 generated runtime bridge 后，bridge 直接调用当前 active slot 中注册的 connect-go handler、grpc-go server、native server、C callback 或 remote adapter。
...
9. connect/grpc remote server adapter 复用标准 connect/grpc client。
