...
- [ ] 添加测试：`cgo_dir` 是合法 rpccgo 参数。
- [ ] 添加测试：空 `cgo_dir` 报明确错误。
- [ ] 添加测试：默认 `cgo_dir` 为 `cgo`。
- [ ] 添加测试：`cgo_dir=../cmd/rpc` 被保存在 generator 配置或 file plan 中。
- [ ] 添加测试：未知参数仍报错。
- [ ] 运行 `rtk go test ./internal/generator -run 'TestGenerate.*CGODir|TestPluginOptions|TestRenderNativeFileFamilyPlan' -count=1`。
- [ ] 验收：测试能明确描述 cgo 输出目录合同，当前实现下失败点清晰。
- [ ] 提交：`test: cover cgo_dir generation contract`
...
- [ ] 增加 generator config，包含 `CGODir`。
- [ ] 默认 `CGODir` 为 `cgo`。
- [ ] `parseRPCCGOParameter` 接受 `cgo_dir=<relative path>`。
- [ ] 空 `cgo_dir` 返回明确错误。
- [ ] 禁止绝对路径，避免 protoc 输出越过生成根目录。
- [ ] 保留标准 `paths` 和 `M...` 参数由 `protogen` 原生处理。
- [ ] 将 `CGODir` 写入 `FilePlan` 或 cgo file family plan。
- [ ] 运行 `rtk go test ./internal/generator -run 'TestGenerate.*CGODir|TestPluginOptions' -count=1`。
- [ ] 验收：generator 能接受并保存 `cgo_dir`，默认值稳定。
- [ ] 提交：`feat: parse cgo_dir generator option`
...
- [ ] 从 `GeneratedFilenamePrefix` 取 protobuf Go package 生成目录。
- [ ] 用 `path.Join(goPackageDir, cgoDir)` 计算 cgo 输出目录。
- [ ] 用 `path.Clean` 清理 `../cmd/rpc` 这类路径。
- [ ] `Runtime` 和 `NativeServer` 继续使用原 `GeneratedFilenamePrefix`。
- [ ] `CGONativeServer` 和 `CGONativeClient` 使用 cgo 输出目录。
- [ ] cgo 文件 stem 保留 proto basename、service name、role，避免同 service 多文件覆盖。
- [ ] 添加测试：默认输出到 `test/v1/cgo/`。
- [ ] 添加测试：`cgo_dir=../cmd/rpc` 输出到清理后的 `test/cmd/rpc/`。
- [ ] 添加测试：非 source-relative paths 模式也能基于 Go package 生成目录计算 cgo 输出路径。
- [ ] 运行 `rtk go test ./internal/generator -run 'TestRenderNativeFileFamilyPlan|TestGenerateWithNativeRenderer' -count=1`。
- [ ] 验收：cgo 文件路径符合 `cgo_dir` 合同，非 cgo 文件不受影响。
- [ ] 提交：`feat: route cgo files through cgo_dir`
...
- [ ] cgo client generated file 输出 `package main`。
- [ ] cgo server generated file 输出 `package main`。
- [ ] cgo files 使用 import alias 引用 protobuf Go package。
- [ ] cgo files 不再假设自己与 generated runtime 同包。
- [ ] 添加 source assertion 测试：cgo 文件包含 `package main`。
- [ ] 添加 source assertion 测试：cgo 文件 import protobuf Go package。
- [ ] 添加 source assertion 测试：runtime/native server 文件仍使用 protobuf Go package name。
- [ ] 运行 `rtk go test ./internal/generator -run 'TestRenderNative.*CGO|TestGenerateWithNativeRenderer' -count=1`。
- [ ] 验收：cgo generated files 的 package 边界符合 `package main` 合同。
- [ ] 提交：`feat: generate cgo files in package main`
...
- [ ] 审计 cgo renderer 使用的 runtime helper、dispatcher helper、encoder/decoder、adapter/session helper。
- [ ] 对 cgo bridge 必须调用的 service runtime 入口做最小导出。
- [ ] 保持内部 helper 未导出，避免扩大 public API 面。
- [ ] cgo client 通过 service package alias 调用 exported dispatcher/client bridge。
- [ ] cgo server registration 通过 service package alias 调用 exported registration bridge。
- [ ] error store 仍使用 `rpcruntime`，error id 保持 `int32`。
- [ ] 添加 compile-oriented source assertion，证明 cgo file 不引用 service package 未导出符号。
- [ ] 运行 `rtk go test ./internal/generator -run 'TestRenderRuntime|TestRenderNative.*CGO' -count=1`。
- [ ] 验收：`package main` cgo 文件不依赖同包未导出符号。
- [ ] 提交：`feat: expose minimal cgo bridge API`
...
- [ ] 添加 fixture：默认 `cgo_dir` 生成到 Go package 的 `cgo/` 子目录。
- [ ] 添加 fixture：`cgo_dir=../cmd/rpc` 生成到 Go package 目录之外。
- [ ] fixture 编译包含 protobuf Go package 和 cgo `package main`。
- [ ] fixture 验证 cgo `package main` 能 import generated service package。
- [ ] fixture 验证 native runtime/native server 文件仍在 protobuf Go package。
- [ ] 不纳入 `.vscode/` 或本机环境 workaround。
- [ ] 运行 `rtk go test ./internal/integration -run 'TestCGODirGeneration' -count=1`。
- [ ] 验收：默认目录和外部目录两种 cgo 生成形态都能编译。
- [ ] 提交：`test: cover cgo_dir compile fixtures`
...
- [ ] 确认 `AGENTS.md` 已记录：cgo 文件必须生成到 `package main`。
- [ ] 确认 `AGENTS.md` 已记录：`cgo_dir` 相对 protobuf Go package 生成目录解析。
- [ ] 确认 `AGENTS.md` 已记录：`cgo_dir` 默认是 Go package 目录下的 `cgo` 子目录。
- [ ] 更新本计划 checkbox。
...
- [ ] 验收：生成器输出布局、package 边界、测试和长期规则一致。
- [ ] 提交：`docs: record cgo_dir main package contract`
