...
- [ ] 定义 message direct path generated file family plan，复用 runtime、client.cgo、server.cgo 文件族。
- [ ] cgo message client 生成策略不受 `@rpccgo` server adapter 注释控制。
- [ ] 只有需要 cgo message server adapter 时才生成 message server callback section。
- [ ] renderer plan 明确不生成 `<service>.codec.rpccgo.go`。
- [ ] 添加测试：message direct path 输出预期文件名。
- [ ] 添加测试：Stage 4A 不输出 connect/grpc/remote/codec 文件。
- [ ] 运行 `rtk go test ./internal/generator ./cmd/protoc-gen-rpc-cgo -run 'TestRenderMessage|TestGenerate' -count=1`。
- [ ] 验收：生成文件布局与架构文档一致，且没有引入旧 framework selector。
- [ ] 提交：`feat: add message direct renderer pipeline`
...
- [ ] 生成 service message adapter interface，覆盖 unary 与三类 streaming operation。
- [ ] 生成 message active server registration helper 的内部接入点，供 cgo message server adapter 使用。
- [ ] 生成 dispatcher message entrypoints，contract 匹配时直接调用 message adapter。
- [ ] contract mismatch 时返回明确错误，错误文本说明 converter 尚未启用。
- [ ] stream `Start` 捕获 active server snapshot，后续 message stream 操作固定路由到该 snapshot。
- [ ] 添加 source assertion 测试：runtime glue import `rpcruntime`，不 import connect/grpc。
- [ ] 添加测试：runtime glue 中 message stream handle 类型是 `rpcruntime.StreamHandle`。
- [ ] 运行 `rtk go test ./internal/generator -run 'TestRenderRuntime' -count=1`。
- [ ] 验收：generated runtime 同时支持 native 和 message contract dispatch，但 message direct path 不依赖 converter。
- [ ] 提交：`feat: render message service runtime glue`
...
- [ ] 生成 cgo message server callback table。
- [ ] 生成 `Register<Service>CGOMessageServer(callbacks)`。
- [ ] nil callback table 或缺失 unary callback 返回明确错误。
- [ ] adapter 调用 C callback，并按 `int32` error id 传播错误。
- [ ] callback request/response 使用 protobuf bytes ABI，不接触 native field wrapper。
- [ ] 添加 generated source assertion 测试，覆盖 unary callback 签名。
- [ ] 添加 integration fixture：cgo message client unary 调用 cgo message server。
- [ ] 添加测试：后注册 cgo message server 覆盖后续调用，已启动 stream 保持旧 snapshot。
- [ ] 运行 `rtk go test ./internal/generator ./internal/integration -run 'TestMessageCGOServerUnary|TestMessageUnary' -count=1`。
- [ ] 验收：cgo message server 可以注册成 active server，message unary direct path 可用。
- [ ] 提交：`feat: render cgo message server callbacks`
...
- [ ] 生成 cgo message unary client export。
- [ ] request bytes 先执行 protobuf unmarshal 校验，再以 message contract 进入 dispatcher。
- [ ] response message marshal 成 protobuf bytes 返回给 cgo message client。
- [ ] Go error 存入 `rpcruntime` error store，并通过 `int32` error id 返回。
- [ ] 缺少 active server 时返回明确 error id。
- [ ] active server 是 native adapter 时返回明确 contract mismatch error，不执行转换。
- [ ] 添加 integration fixture：cgo message client unary 调用 cgo message server。
- [ ] 添加测试：invalid request bytes、marshal/unmarshal error、callback error propagation。
- [ ] 运行 `rtk go test ./internal/generator ./internal/integration -run 'TestMessageUnary' -count=1`。
- [ ] 验收：cgo message client unary 通过 dispatcher 直连 cgo message server。
- [ ] 提交：`feat: route message unary client to message server`
...
- [ ] 生成 client streaming `Start`、`Send`、`Finish`、`Cancel` message ABI。
- [ ] `Start` 捕获 active server snapshot 并返回 `rpcruntime.StreamHandle`。
- [ ] `Send` 校验 request protobuf bytes，使用 handle 找回 session。
- [ ] `Finish` terminal 操作使用 typed take，成功后 handle 不再可用。
- [ ] `Cancel` 传播到底层 message adapter 并 finalize。
- [ ] 添加 integration fixture：client streaming 到 cgo message server。
- [ ] 添加测试：invalid send bytes、send closed、finalized、canceled 后返回明确错误。
- [ ] 运行 `rtk go test ./internal/integration -run 'TestMessageClientStreaming' -count=1`。
- [ ] 验收：client streaming message direct path 复用统一 stream lifecycle。
- [ ] 提交：`feat: support message client streaming direct path`
...
- [ ] 生成 server streaming `Start`、`Cancel`、`onRead`、`onDone` message ABI。
- [ ] `Start` 校验 request protobuf bytes 并返回 `rpcruntime.StreamHandle`。
- [ ] `onRead` 通过 handle 读取固定 snapshot session，并返回 response protobuf bytes。
- [ ] `onDone` 使用 typed take，并执行 terminal cleanup。
- [ ] `Cancel` 传播到底层 message adapter 并 finalize。
- [ ] 添加 integration fixture：server streaming 到 cgo message server。
- [ ] 添加测试：onDone 后 handle 不再可用，callback error 进入 error store。
- [ ] 运行 `rtk go test ./internal/integration -run 'TestMessageServerStreaming' -count=1`。
- [ ] 验收：server streaming message direct path 在 onDone 后释放 handle。
- [ ] 提交：`feat: support message server streaming direct path`
...
- [ ] 生成 bidi streaming `Start`、`Send`、`CloseSend`、`Cancel`、`onRead`、`onDone` message ABI。
- [ ] `Send` 校验 request protobuf bytes，`onRead` 返回 response protobuf bytes。
- [ ] `CloseSend` 后继续 `Send` 返回明确错误。
- [ ] `onDone` 后所有 handle 操作返回稳定错误或 false。
- [ ] `Cancel` 不重复执行 callback。
- [ ] 添加 integration fixture：bidi streaming 到 cgo message server。
- [ ] 添加测试：invalid send bytes、callback error、cancel 后 read/done 行为。
- [ ] 运行 `rtk go test ./internal/integration -run 'TestMessageBidiStreaming' -count=1`。
- [ ] 验收：bidi message direct path 覆盖 send/read/close/cancel/done 全生命周期。
- [ ] 提交：`feat: support message bidi streaming direct path`
...
- [ ] 添加 Stage 4A acceptance test，覆盖 cgo message client 到 cgo message server。
- [ ] acceptance test 覆盖 unary、client streaming、server streaming、bidi streaming。
- [ ] acceptance test 证明 cgo message client 调用都进入 dispatcher。
- [ ] acceptance test 证明 active server 是 native adapter 时返回 contract mismatch error。
- [ ] 写入已迁移、参考后重写、不迁移清单。
- [ ] 明确旧多 registry、多 provider bootstrap、framework selector 不迁移。
- [ ] 明确 Stage 4A 不生成 `<service>.codec.rpccgo.go`，不实现 native/message converter。
...
- [ ] 验收：阶段 4A “迁移了什么、为什么参考后重写、为什么不迁旧架构和 converter”有明确记录。
- [ ] 提交：`docs: record stage 4a migration inventory`
